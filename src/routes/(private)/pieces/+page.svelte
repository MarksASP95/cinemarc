<script lang="ts">
  ...
  const filterKeyIsForDBMap: Record<
    keyof PieceFixedValueFilter,
    boolean | undefined
  > = {
  ...
  function filterPiecesBySearch(
    ...
    return pieces.filter((piece) =>
      piece.name.toLocaleLowerCase().includes(_searchStr)
    );
  }
  ...
  function applyYearFilters(pieces: Piece[]): Piece[] {
    return pieces.filter((piece) => {
      ...
      return (
        piece.releaseYear >= (currentFilters.releaseYearStart || 0) &&
  ...
  function handleFiltersFormSubmit({
    ...
    filters: PieceFixedValueFilter;
    changedKeys: (keyof PieceFixedValueFilter)[];
  }) {
  ...
  function handlePieceDeleted(
    e: CustomEvent<{ pieceId: string; isDeleted: boolean }>
  ) {
  ...
  function handlePieceConsumed(
    e: CustomEvent<{ pieceId: string; consumed: boolean }>
  ) {
...
<section class="pieces-page">
  <div
    class:show={!!undoDeleteFn}
    class="undo-popup btn btn-sm bg-gradient-to-br variant-gradient-error-warning"
  >
    Piece deleted &nbsp; <button
      on:click={() => (undoDeleteFn ? undoDeleteFn() : null)}
      type="button"
      class="btn btn-sm variant-ghost-success">undo</button
    >
  </div>
  <div
    class:undo-popup--second={!!undoDeleteFn}
    class:show={!!undoConsumedFn}
    class="undo-popup btn btn-sm bg-gradient-to-br variant-gradient-primary-tertiary"
  >
    Marked as consumed &nbsp; <button
      on:click={() => (undoConsumedFn ? undoConsumedFn() : null)}
      type="button"
      class="btn btn-sm variant-ghost-success">undo</button
    >
  </div>
  ...
  {#if !displayedPieces}
    <div class={searchGridClasses}>
      <div
        class="placeholder animate-pulse col-span-10 md:col-span-11"
        style="height: 42px"
      ></div>
      <div
        class="placeholder animate-pulse col-span-2 md:col-span-1"
        style="height: 42px"
      ></div>
    </div>
  {/if}
  ...
  {#if !!pieces?.length}
    <div class={searchGridClasses}>
      ...
      >
        <svg
          fill={filtersActive ? "#fff" : undefined}
          stroke={filtersActive ? "#fff" : undefined}
          width="24"
          height="24"
  ...
  {#if filtersActive}
    <PieceFiltersBadges show={true} {currentFilters} fixedFilters={false} />
    <PieceFiltersBadges
      show={fixedFilters}
      {currentFilters}
      fixedFilters={true}
  ...
  {#if pieces?.length === 0}
    <div class="p4 mt-16">
      {#if filtersActive && !showPiecesPlaceholders}
        <p class="text-center mt-4 mb-4" style="font-size: 4rem;">🫥</p>
        ...
        <div class="flex justify-center mb-4">
          <button
            on:click={openFiltersModal}
            type="button"
            class="btn w-full max-w-xs btn-sm variant-filled clear-search-button"
          >
            Change filters
          </button>
        </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
    {#if !!displayedPieces && !showPiecesPlaceholders}
      {#each displayedPieces as piece, index (piece.id)}
        <PieceCard
          on:pieceConsumedToggle={handlePieceConsumed}
          on:pieceDeletedToggle={handlePieceDeleted}
          on:editButtonClick={openEditModal}
          {piece}
          {index}
        />
      {/each}
      ...
      <button
        on:click={handlePlusButtonClick}
        type="button"
        class="add-piece-btn btn-icon btn-icon-xl bg-gradient-to-br variant-gradient-tertiary-primary fixed bottom-6 right-6"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
    ...
    {:else}
      {#each [0, 0, 0, 0, 0, 0, 0, 0] as _}
        <div
          class="card bg-initial relative piece-card cursor-pointer animate-pulse"
        >
    ...
    <button
      class:shown={showGoToTopButton}
      on:click={handleGoToTopButtonClick}
      type="button"
      class="go-to-top-btn btn-icon btn-icon-xl variant-filled fixed left-6"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
...
<style lang="scss">
  ...
  .undo-popup {
    ...
    z-index: 9999;
    transition: top 200ms cubic-bezier(0.31, 0.94, 1, 0.99);
  ...
  .go-to-top-btn {
    bottom: -4.5rem;
    opacity: 0;
    transition: all 200ms cubic-bezier(0.31, 0.94, 1, 0.99);
    &.shown {
      bottom: 1.5rem;
      opacity: 1;
    }
